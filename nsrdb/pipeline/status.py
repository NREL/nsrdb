...
class Status(dict):
    """Base class for NSRDB data pipeline health and status information."""
    ...
    def _parse_fpath(status_dir, name):
        ...
        if name is None:
            fpath = os.path.join(status_dir, 'nsrdb_status.json')
    ...
    def _dump(self):
        ...
        ----------
        data : dict
            NSRDB data pipeline status info.
        """
        self._sort_by_index()
    ...
    def _update_job_status(self, module, job_name, hardware='local'):
        ...
        ----------
        module : str
            NSRDB module that the job belongs to.
        job_name : str
    ...
    def _set_job_status(self, module, job_name, status):
        ...
        if module not in self.data:
            raise KeyError('NSRDB pipeline status has not been initialized '
                           'for "{}".'.format(module))
    ...
    def add_job(cls, status_dir, module, job_name, replace=False,
        ...
        exists = obj.job_exists(status_dir, job_name, module=module)
    ...
    @classmethod
    def job_exists(cls, status_dir, job_name, module=None):
        ...
        job_name : str
            Unique job name identification.
        module : str
            Optional job module to look in for job_name.
        ...
        if obj.data:
            if module is None:
                job_module = obj.data.values()
            else:
                job_module = obj.data[module]

        if job_module:
            for name in job_module.keys():
                if name == job_name:
                    exists = True
                    break
