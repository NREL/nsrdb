...
from nsrdb.utilities.execution import SubprocessManager, SLURM
...
class Pipeline:
            ...
            if return_code == 0:
                ...
                while return_code == 1 and self.monitor:
                    ...
                    if return_code == 2:
                        ...
                        raise RuntimeError('NSRDB pipeline failed at step '
                                           '{} "{}" {}'
    ...
    def _check_jobs_submitted(status, module):
        ...
        Parameters
        ----------
        status : nsrdb.pipeline.status.Status
            NSRDB job status object.
        module : str
            NSRDB module.
    ...
    def _get_status_obj(self):
        """Get a NSRDB pipeline status object.
        ...
        status = Status(self._config['out_dir'], name=self._config['name'])
        return status
    ...
    def _get_cmd(command, f_config):
        ...
        ----------
        command : str
            NSRDB cli command which should be a NSRDB module.
        f_config : str
    ...
    def _get_job_status(module_status, option='all'):
        ...
        ----------
        module_status : dict
            Status dictionary for a full NSRDB module containing one or more
            job status dict.
        option : str
    ...
    def parse_previous(status_dir, module, target='fpath', target_module=None):
        ...
        msg = ('Could not parse data regarding "{}" from NSRDB status file in '
        ...
        if target == 'fpath':
            for status in job_statuses:
                out.append(os.path.join(status['out_dir'], status['fout']))
        else:
            for status in job_statuses:
