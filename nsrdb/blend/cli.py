...
TODO: Refactor with new CLI. Setup to interface with config file instead of
lots of args. Call from nsrdb.cli
"""
...
@click.option(
    '--east_dir', '-ed', type=STR, required=True, help='Source east directory.'
)
@click.option(
    '--west_dir', '-wd', type=STR, required=True, help='Source west directory.'
)
@click.option(
    '--out_fn', '-of', type=STR, default=None, help='Optional output filename.'
)
@click.option(
    '--east_fn',
    '-ef',
...
def main(
    ...
    if ctx.invoked_subcommand is None:
        ...
        logger.info(
            'NSRDB Blend complete. Time elapsed: {:.2f} min. Target '
...
def slurm(ctx, alloc, walltime, feature, memory, stdout_path):
    ...
    for name, cmd in zip(names, cmds):
        ...
        if out:
            msg = 'Kicked off nsrdb blend job "{}" (SLURM jobid #{}).'.format(
                name, out
            )
        else:
            msg = (
                'Was unable to kick off nsrdb blend job "{}". '
