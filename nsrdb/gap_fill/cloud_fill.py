...
class CloudGapFill:
    ...
    def fill_cloud_cat(category, cloud_prop, cloud_type):
        ...
        cloud_prop_fill = (
            cloud_prop_fill.interpolate(method='nearest', axis=0)
            .ffill()
            .bfill()
        )
    ...
    def handle_persistent_nan(dset, cloud_prop, cloud_type):
        ...
        if np.sum(np.isnan(cloud_prop.values)) > 0:
            ...
            msg = (
                'NaN values persist in "{}" at {} sites out of {} '
                '({:.2f}%) - {} total NaN values '
                'out of {} total observations, {:.2f}%).'.format(
                    dset,
                    len(loc),
                    cloud_prop.shape[1],
            ...
            logger.debug(
                'These site indices have persistent NaN values: {}'.format(ind)
            )
            ...
            for cat, types in CloudGapFill.CATS.items():
                ...
                msg = (
                    'Filling {} persistent NaN values of "{}" with '
                    'a climatalogical average value {} for cloud '
                    'category "{}".'.format(
                        mask.values.sum(), dset, fill_val, cat
                    )
                )
                logger.warning(msg)
                warn(msg)
    ...
    def fill_cloud_type(cloud_type, fill_flag=None, missing=-15):
        ...
        if missing_mask.all(axis=0).any():
            ...
            msg = (
                '{} sites have missing cloud types for the '
        ...
        if missing_mask.any():
            ...
            if any(one_good):
                cloud_type.iloc[:, one_good] = (
                    cloud_type.iloc[:, one_good].ffill().bfill()
                )
            cloud_type = (
                cloud_type.interpolate(method='nearest', axis=0)
                .ffill()
                .bfill()
            )
        ...
        if missing_mask.sum() > 0:
            e = (
                'Cloud type fill failed! {} cloud types are '
                'still missing out of shape {}'.format(
                    missing_mask.sum(), missing_mask.shape
                )
            )
            logger.error(e)
            raise RuntimeError(e)

        bad = set(np.unique(cloud_type.values)) - set(
            CLOUD_TYPES + CLEAR_TYPES
        )
        if any(bad):
            e = (
                'Unknown cloud types have appeared! These are not '
    ...
    @staticmethod
    def flag_missing_properties(
        prop_name, cloud_prop, cloud_type, sza, fill_flag
    ):
        ...
        """
        missing_prop = (
            cloud_type.isin(CLOUD_TYPES).values
            & (cloud_prop <= 0).values
            & (sza < SZA_LIM).values
        )
        ...
        logger.debug(
            'Cloud property "{}" is being cleaned with {} '
            'fill_flag=3 out of {} total observations ({:.2f}%)'.format(
                prop_name,
                mask.sum(),
                mask.shape[0] * mask.shape[1],
                100 * mask.sum() / (mask.shape[0] * mask.shape[1]),
            )
        )
        ...
        if missing_full.all(axis=0).any():
            ...
            logger.debug(
                'Cloud property "{}" is being cleaned with {} '
                'fill_flag=4 out of {} total observations ({:.2f}%)'.format(
                    prop_name,
                    mask_full.sum(),
            ...
            logger.debug(
                'Cloud property "{}" is being cleaned with {} sites '
                'with fill_flag=4 out of {} total sites ({:.2f}%)'.format(
                    prop_name,
                    mask_full.any(axis=0).sum(),
                    mask_full.shape[1],
                    100 * mask_full.any(axis=0).sum() / mask_full.shape[1],
                )
            )
    ...
    def fill_cloud_prop(
        ...
        if not cloud_type_is_clean:
            # fill cloud types.
            cloud_type, fill_flag = cls.fill_cloud_type(
                cloud_type, fill_flag=fill_flag
            )

        fill_flag = cls.flag_missing_properties(
            prop_name, cloud_prop, cloud_type, sza, fill_flag
        )
        ...
        cloud_prop = cls.handle_persistent_nan(
            prop_name, cloud_prop, cloud_type
        )
    ...
    def fill_file(
        ...
        if col_chunk is None:
            ...
            logger.info(
                'Gap fill will be run across the full data column '
                'shape {} in {} column chunks with approximately {} '
                'sites per chunk'.format(len(columns), len(slices), col_chunk)
            )
        ...
        for cols in slices:
            ...
            with Outputs(f_cloud, mode='a') as f:
                logger.debug(
                    'Writing filled cloud types to {}'.format(
                        os.path.basename(f_cloud)
                    )
                )
            ...
            with Outputs(f_cloud, mode='a') as f:
                logger.debug(
                    'Writing fill flag to {}'.format(os.path.basename(f_cloud))
                )
