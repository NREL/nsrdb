...
def missing_cld_props(cloud_type, cld_opd_dcomp, cld_reff_dcomp):
    ...
    missing_props = (np.isin(cloud_type, CLOUD_TYPES)) & (
        (cld_opd_dcomp <= 0)
        | (cld_reff_dcomp <= 0)
        | (np.isnan(cld_opd_dcomp) | (np.isnan(cld_reff_dcomp)))
    )
...
def make_fill_flag(
    irrad, cs_irrad, cloud_type, missing_cld_props, cloud_fill_flag=None
):
...
def gap_fill_irrad(
    irrad, cs_irrad, fill_flag, return_csr=False, flags_to_fill=None
):
...
def enforce_clearsky(
    dni, ghi, cs_dni, cs_ghi, sza, fill_flag, sza_lim=SZA_LIM
):
